<template><div><h1 id="nextjs-14에서-데이터-fetch-하는-방법" tabindex="-1"><a class="header-anchor" href="#nextjs-14에서-데이터-fetch-하는-방법" aria-hidden="true">#</a> Nextjs 14에서 데이터 fetch 하는 방법</h1>
<p>Next.js는 네이티브 Web fetch() API를 확장하여 서버의 각 요청이 자체 지속적 캐싱 의미론을 설정할 수 있도록 합니다.</p>
<p>브라우저에서 cache 옵션은 fetch 요청이 브라우저의 HTTP 캐시와 상호 작용하는 방법을 나타냅니다. 이 확장 기능을 통해 cache는 서버 측 fetch 요청이 프레임워크의 지속적인 HTTP 캐시와 상호 작용하는 방법을 나타냅니다.</p>
<p>서버 컴포넌트 내에서 async 및 await을 사용하여 fetch를 직접 호출할 수 있습니다.</p>
<!-- ui-log 수평형 -->
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="fetch-url-options" tabindex="-1"><a class="header-anchor" href="#fetch-url-options" aria-hidden="true">#</a> fetch(url, options)</h2>
<p>Next.js는 Web fetch() API를 확장하므로 사용 가능한 네이티브 옵션을 모두 사용할 수 있습니다.</p>
<h3 id="options-cache" tabindex="-1"><a class="header-anchor" href="#options-cache" aria-hidden="true">#</a> options.cache</h3>
<!-- ui-log 수평형 -->
<p><ins class="adsbygoogle"
      style="display:block"
      data-ad-client="ca-pub-4877378276818686"
...
<p>Next.js 데이터 캐시와 상호 작용하는 요청을 구성하세요.</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token function">fetch</span><span class="token punctuation">(</span><span class="token template-string"><span class="token template-punctuation string">`</span><span class="token string">https://...</span><span class="token template-punctuation string">`</span></span><span class="token punctuation">,</span> <span class="token punctuation">{</span> <span class="token literal-property property">cache</span><span class="token operator">:</span> <span class="token string">"force-cache"</span> <span class="token operator">|</span> <span class="token string">"no-store"</span> <span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><ul>
<li>force-cache (기본 설정) - Next.js는 데이터 캐시에서 일치하는 요청을 찾습니다. 일치하고 새로운 경우, 캐시에서 반환됩니다. 일치하지 않거나 오래된 경우, Next.js는 리소스를 원격 서버에서 가져와 다운로드된 리소스로 캐시를 업데이트합니다.</li>
<li>일치하고 새로운 경우, 캐시에서 반환됩니다.</li>
<li>일치하지 않거나 오래된 경우, Next.js는 리소스를 원격 서버에서 가져와 다운로드된 리소스로 캐시를 업데이트합니다.</li>
<li>no-store - Next.js는 캐시를 확인하지 않고 모든 요청마다 원격 서버에서 리소스를 가져오며, 다운로드된 리소스로 캐시를 업데이트하지 않습니다.</li>
</ul>
<blockquote>
<p>알아두면 좋은 사실:
캐시 옵션을 제공하지 않으면 Next.js는 기본적으로 force-cache로 설정됩니다. cookies()와 같은 동적 함수를 사용한 경우에는 no-store로 설정됩니다. no-cache 옵션은 Next.js에서 no-store와 동일하게 작동합니다.</p>
</blockquote>
<!-- ui-log 수평형 -->
<p><ins class="adsbygoogle"
      style="display:block"
...
<h3 id="options-next-revalidate" tabindex="-1"><a class="header-anchor" href="#options-next-revalidate" aria-hidden="true">#</a> options.next.revalidate</h3>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token function">fetch</span><span class="token punctuation">(</span><span class="token template-string"><span class="token template-punctuation string">`</span><span class="token string">https://...</span><span class="token template-punctuation string">`</span></span><span class="token punctuation">,</span> <span class="token punctuation">{</span> <span class="token literal-property property">next</span><span class="token operator">:</span> <span class="token punctuation">{</span> <span class="token literal-property property">revalidate</span><span class="token operator">:</span> <span class="token boolean">false</span> <span class="token operator">|</span> <span class="token number">0</span> <span class="token operator">|</span> number <span class="token punctuation">}</span> <span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>리소스의 캐시 지속 시간(초)을 설정합니다.</p>
<ul>
<li>false - 리소스를 무기한 캐싱합니다. revalidate: Infinity 와 의미적으로 같습니다. HTTP 캐시는 오래된 리소스를 시간이 지남에 따라 제거할 수 있습니다.</li>
<li>0 - 리소스가 캐싱되지 않도록 방지합니다.</li>
<li>number - (초) 리소스가 최대 n 초 동안 캐시 지속 시간이 있어야 함을 지정합니다.</li>
</ul>
<!-- ui-log 수평형 -->
<p><ins class="adsbygoogle"
      style="display:block"
...
편의상, revalidate가 숫자로 설정된 경우 cache 옵션을 설정할 필요가 없습니다. 0은 cache: <code v-pre>no-store</code>를 의미하고 양수 값은 cache: <code v-pre>force-cache</code>를 의미합니다.
{ revalidate: 0, cache: <code v-pre>force-cache</code> } 나 { revalidate: 10, cache: <code v-pre>no-store</code> }와 같이 충돌하는 옵션이 있는 경우 오류가 발생합니다.</p>
</blockquote>
<h3 id="options-next-tags" tabindex="-1"><a class="header-anchor" href="#options-next-tags" aria-hidden="true">#</a> options.next.tags</h3>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>리소스의 캐시 태그를 설정합니다. 그런 다음 revalidateTag를 사용하여 필요할 때 데이터를 다시 유효화할 수 있습니다. 사용자 정의 태그의 최대 길이는 256자이며 최대 태그 항목 수는 64입니다.</p>
<!-- ui-log 수평형 -->
<p><ins class="adsbygoogle"
      style="display:block"
      data-ad-client="ca-pub-4877378276818686"
...
<td><code v-pre>fetch</code>가 도입되었습니다.</td>
</tr>
</tbody>
</table>
<!-- ui-log 수평형 -->
